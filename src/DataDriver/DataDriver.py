...
class DataDriver:
    """
        ...
        DataDriver creates multiple test cases based on a test template and data contend of a CSV file.
        All created tests share the same test sequence (keywords) and differs in the test data.
        ...
        After activation it searches for the ``Test Template`` -Keyword to analyse the ``[Arguments]`` is has.
        As second step it loads the data from the specified CSV file.
        ...
        === Variable types ===
        In this early Version of Data Driver only acalar variables are supported.
        ...
        In this example the Data Driver is activate by using it as Library. It is used with default settings.

        As ``Test Template`` the keyword ``Invalid Login`` is used. This keyword has two arguments.
        Argument names are _${username}_ and _${password}_. These names has to be in the CSV file as column header.
        ...
        The first two test cases has specific names. The other six test cases will generate names based on
        ...
        Default is Excel-EU which is a semicolon separated file.
