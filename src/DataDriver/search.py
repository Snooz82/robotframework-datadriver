...
class VariableMatch(object):
    ...
    def resolve_base(self, variables, ignore_errors=False):
        if self.identifier:
            ...
            self.base = variables.replace_string(
                internal, custom_unescaper=unescape_variable_syntax, ignore_errors=ignore_errors,
            )
    ...
    def is_variable(self):
        return bool(
            self.identifier and self.base and self.start == 0 and self.end == len(self.string)
        )
...
class VariableSearcher(object):
    ...
    def _start_index_is_ok(self, string, index):
        return string[index] in self.identifiers and not self._is_escaped(string, index)
